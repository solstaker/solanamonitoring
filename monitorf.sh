...
binDir="/opt/solana/testnet/bin/"              # auto detection of the solana binary directory can fail or an alternative custom installation is preferred, in case insert like $HOME/solana/target/release
rpcURL=$(cat $configDir/cli/config.yml | grep json_rpc_url | grep -o '".*"' | tr -d '"')              # default is localhost with port number autodiscovered, alternatively it can be specified like http://custom.rpc.com:port
...
cpu=$(lscpu | grep " Model name:" | cut -c 18- |  sed 's/^ *//g')
